...
internal class TemplateRepositorySettings(
        ...
        scope.launch {
            withContext(dispatcher) {
                val loadedTemplates = jsonData?.let { jsonData ->
                    Json.Default.decodeFromString<List<Template>>(jsonData)
                } ?: emptyList()
    ...
    override fun importFromJson(json: String, replace: Boolean) {
        try {
            val importedTemplates = Json.Default.decodeFromString<List<Template>>(json)
